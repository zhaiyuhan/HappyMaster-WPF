...
namespace Lyric
{
    ...
    class GetLyricAndLyricTime
    {
        ...
        #endregion
        public string string2UTF8(string defStr, System.Text.Encoding srcEncoding, System.Text.Encoding dstEncoding)
        {
            if (string.IsNullOrEmpty(defStr))
                return defStr;

            byte[] defBytes = srcEncoding.GetBytes(defStr);
                        ...
                        if (!string.IsNullOrEmpty(time))
                        {
                            try
                            {
                                ...
                                if (!lyricAndTimeDictionary.ContainsKey(currentTime))
                                {
                                    string result = null;
                                    // default to utf8 
                                    result = string2UTF8(correctLyricTxt, System.Text.Encoding.Default, System.Text.Encoding.UTF8);
                                    lyricAndTimeDictionary.Add(currentTime, result);
                                }
                            }
                            catch
                            {
